...
int platform_ipc_init(struct ipc *ipc)
{
	 ...
	 */
	imx_mu_xcr_rmw(IMX_MU_VERSION, IMX_MU_GIER, IMX_MU_xCR_GIEn(IMX_MU_VERSION, 0) | IMX_MU_xCR_GIEn(IMX_MU_VERSION, 1), 0);
...
void ipc_platform_poll_set_cmd_done(void)
{
	...
	/* request GP interrupt #0 - notify host that reply is ready */
	imx_mu_xcr_rmw(IMX_MU_VERSION, IMX_MU_GCR, IMX_MU_xCR_GIRn(IMX_MU_VERSION, 0), 0);
	tr_info(&ipc_tr, "ipc_platform_poll_set cmd_done");
}
...
int ipc_platform_poll_tx_host_msg(struct ipc_msg *msg)
{
	...
	tr_info(&ipc_tr, "ipc_platform_poll_tx_host_msg");
	/* message sent */
	platform_shared_commit(msg, sizeof(*msg));
	return 1;
}
