...
static void irq_handler(void *arg)
{
	...
	if (status & IMX_MU_xSR_GIPn(1)) {
		...
		imx_mu_xcr_rmw(IMX_MU_VERSION, IMX_MU_GIER, 0, IMX_MU_xCR_GIEn(1));

		/* Clear GP pending interrupt #1 */
		imx_mu_xsr_rmw(IMX_MU_VERSION, IMX_MU_GSR, IMX_MU_xSR_GIPn(1), 0);
		...
		imx_mu_xcr_rmw(IMX_MU_VERSION, IMX_MU_GIER, IMX_MU_xCR_GIEn(1), 0);
	}
	...
	if (status & IMX_MU_xSR_GIPn(0)) {
		...
		imx_mu_xcr_rmw(IMX_MU_VERSION, IMX_MU_GIER, 0, IMX_MU_xCR_GIEn(0));

		/* Clear GP pending interrupt #0 */
		imx_mu_xsr_rmw(IMX_MU_VERSION, IMX_MU_GCR, IMX_MU_xSR_GIPn(0), 0);
...
void ipc_platform_complete_cmd(void *data)
{
	...
	imx_mu_xcr_rmw(IMX_MU_VERSION, IMX_MU_GIER, IMX_MU_xCR_GIEn(0), 0);

	/* request GP interrupt #0 - notify host that reply is ready */
	imx_mu_xcr_rmw(IMX_MU_VERSION, IMX_MU_GCR, IMX_MU_xCR_GIRn(0), 0);
...
int ipc_platform_send_msg(struct ipc_msg *msg)
{
	...
	if (ipc->is_notification_pending ||
	    imx_mu_read(IMX_MU_xCR(IMX_MU_VERSION, IMX_MU_GCR) & IMX_MU_xCR_GIRn(1)) {
		ret = -EBUSY;
		goto out;
	}
...
int platform_ipc_init(struct ipc *ipc)
{
	 ...
	 */
	imx_mu_xcr_rmw(IMX_MU_VERSION, IMX_MU_GIER, IMX_MU_xCR_GIEn(0) | IMX_MU_xCR_GIEn(1), 0);
...
int ipc_platform_poll_is_cmd_pending(void)
{
	...
	if (status & IMX_MU_xSR_GIPn(0)) {
		...
		/* Clear GP pending interrupt #0 */
		imx_mu_xsr_rmw(IMX_MU_VERSION, IMX_MU_GSR, IMX_MU_xSR_GIPn(0), 0);
...
{
	/* can't send notification when one is in progress */
	if (imx_mu_read(IMX_MU_xCR(IMX_MU_VERSION, IMX_MU_GCR)) & IMX_MU_xCR_GIRn(1))
		return 0;
